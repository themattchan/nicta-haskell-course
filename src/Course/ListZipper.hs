...
data ListZipper a =
  ListZipper { lefts :: List a, focus :: a, rights :: List a }
  deriving Eq
...
toList ::
  ListZipper a
  -> List a
toList (ListZipper ls m rs) = reverse ls ++ m:.rs
...
withFocus ::
  ...
  -> ListZipper a
  -> ListZipper a
withFocus f z = z { focus = f (focus z) }
...
findLeft f (ListZipper ls m rs) =
  let (ns,ys) = break f rs in
  case ys of
    Nil -> IsNotZ
    y:.ys -> IsZ $ ListZipper ys y (reverse ns ++ m:.rs)
...
findRight f (ListZipper ls m rs) =
    ...
    case ys of
      Nil -> IsNotZ
      y:.ys -> IsZ $ ListZipper (reverse ns ++ m:.ls) y ys

-- the tests seem to be backwards....
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop (ListZipper (l:.ls) m rs)
  = ListZipper ls l (m:.rs)
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop (ListZipper ls m (r:.rs))
  = ListZipper (m:.ls) r rs
...
moveLeft ::
  ListZipper a
  -> MaybeListZipper a
moveLeft (ListZipper ls m rs) =
  case ls of
    Nil -> IsNotZ
    (l:._) -> IsZ $ ListZipper ls l (m:.rs)
...
moveRight ::
  ListZipper a
  -> MaybeListZipper a
moveRight (ListZipper ls m rs) =
  case rs of
    Nil -> IsNotZ
    (r:._) -> IsZ $ ListZipper (m:.ls) r rs
...
swapLeft z =
  case lefts z of
    Nil -> IsNotZ
    (l:.ls) -> IsZ $ z { lefts = focus z :. ls,  focus = l }
...
swapRight z =
  case rights z of
    Nil -> IsNotZ
    (r:.rs) -> IsZ $ z { focus = r, rights = focus z :. rs }
...
dropLefts ::
  ListZipper a
  -> ListZipper a
dropLefts zipper = zipper { lefts = Nil }
...
dropRights ::
  ListZipper a
  -> ListZipper a
dropRights zipper = zipper { rights = Nil }
